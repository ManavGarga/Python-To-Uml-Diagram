...
def render_graphviz(classes: dict, output_file="uml_diagram"):
    ...
    dot.attr(rankdir="LR")  # optional: make diagram horizontal
    ...
    for cls in classes.values():
        for base in cls.bases:
            if base in classes:
                dot.edge(base, cls.name, arrowhead="onormal")  # inheritance
        for comp in cls.compositions:
            if comp in classes:
                dot.edge(cls.name, comp, style="dashed", label="uses")  # composition

    dot.render(output_file, view=True)  # saves as .png and opens it
